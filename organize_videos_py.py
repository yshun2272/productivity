"""
Video Organization Script - Simple Version

This script reads a Markdown file (videos.md) to rename video files
that have simple numeric names (like 1.mp4, 2.mp4, etc.) and organize
them into folders based on the "Area" column.

Required columns in Markdown table:
- "Current File Name": The number of the file (with or without .mp4 extension)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('video_organization.log'),
...
def organize_videos():
    """Main function to organize videos based on Markdown data."""
    ...
    # Get current directory (should be Videos folder)
    videos_dir = os.getcwd()
    markdown_file = os.path.join(videos_dir, "videos.md")
    ...
    try:
        ...
        for index, row in enumerate(file_data):
            try:
                ...
                # Handle numeric file names with potential leading zeros (01.mp4, 1.mp4, etc.)
                ...
                # Ensure file has .mp4 extension (for videos)
                if not current_file.lower().endswith('.mp4'):
                    current_file = base_file + ".mp4"
                else:
                ...
                # Construct paths
                current_path = os.path.join(videos_dir, current_file)
                area_dir = os.path.join(videos_dir, area)
                ...
                if "Date" in row and row["Date"].strip():
                    ...
                    metadata_commands.append(f"-CreateDate='{date_str}'")
        ...
        if error_count > 0:
            ...
            with open('video_errors.txt', 'w') as f:
                f.write(f"Video organization errors - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
                for err in error_files:
                    f.write(f"- {err}\n")
                    
            print(f"\nDetailed error list saved to 'video_errors.txt'")
            
        logging.info(f"Video organization complete. Success: {success_count}, Errors: {error_count}")
